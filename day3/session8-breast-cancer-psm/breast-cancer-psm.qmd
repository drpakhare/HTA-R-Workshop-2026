...
cat("=== State Occupancy at Key Time Points ===\n\n")
cat("CONTROL (Chemo alone):\n")
cat("  Year 5:  PF =", round(prop_pf_control[6], 3),
    "  PD =", round(prop_pd_control[6], 3),
    "  Dead =", round(prop_dead_control[6], 3), "\n")
cat("  Year 10: PF =", round(prop_pf_control[11], 3),
    "  PD =", round(prop_pd_control[11], 3),
    "  Dead =", round(prop_dead_control[11], 3), "\n")
cat("  Year 20: PF =", round(prop_pf_control[21], 3),
    "  PD =", round(prop_pd_control[21], 3),
    "  Dead =", round(prop_dead_control[21], 3), "\n\n")

cat("TRASTUZUMAB + CHEMO:\n")
cat("  Year 5:  PF =", round(prop_pf_intervention[6], 3),
    "  PD =", round(prop_pd_intervention[6], 3),
    "  Dead =", round(prop_dead_intervention[6], 3), "\n")
cat("  Year 10: PF =", round(prop_pf_intervention[11], 3),
    "  PD =", round(prop_pd_intervention[11], 3),
    "  Dead =", round(prop_dead_intervention[11], 3), "\n")
cat("  Year 20: PF =", round(prop_pf_intervention[21], 3),
    "  PD =", round(prop_pd_intervention[21], 3),
    "  Dead =", round(prop_dead_intervention[21], 3), "\n")
```
...
# Average proportion between start and end of each cycle
...
cat("=== Results (per patient) ===\n\n")
cat("CONTROL (Chemo alone):\n")
cat("  Total discounted cost: ₹", format(round(total_cost_control), big.mark = ","), "\n")
cat("  Total discounted QALYs:", round(total_qaly_control, 3), "\n\n")

cat("TRASTUZUMAB + CHEMO:\n")
cat("  Total discounted cost: ₹", format(round(total_cost_intervention), big.mark = ","), "\n")
cat("  Total discounted QALYs:", round(total_qaly_intervention, 3), "\n")
```
...
cat("Incremental QALYs:", round(inc_qaly, 3), "\n")
cat("ICER: ₹", format(round(icer), big.mark = ","), "per QALY gained\n\n")

# Compare with published Indian estimates
cat("Published Indian ICER (Shrestha et al. 2020):\n")
cat("  ₹1,34,413 to ₹1,78,877 per QALY (1-year trastuzumab)\n\n")

# WTP assessment
wtp_1gdp <- 170000
if (icer < wtp_1gdp) {
  cat("→ Cost-effective at 1x GDP per capita (₹1,70,000/QALY)\n")
} else if (icer < 3 * wtp_1gdp) {
  cat("→ Cost-effective at 3x GDP per capita (₹5,10,000/QALY)\n")
} else {
...
cat("=== Life-Year and QALY Breakdown (undiscounted, per patient) ===\n\n")
cat("                     Chemo Alone    Trastuzumab\n")
cat("Total life-years:   ", round(ly_control, 2), "          ",
    round(ly_intervention, 2), "\n")
cat("Years in PF:        ", round(pf_years_control, 2), "          ",
    round(pf_years_intervention, 2), "\n")
cat("Years in PD:        ", round(pd_years_control, 2), "          ",
    round(pd_years_intervention, 2), "\n")
cat("LY gained:          ", round(ly_intervention - ly_control, 2), "\n")
cat("PF years gained:    ", round(pf_years_intervention - pf_years_control, 2), "\n")
```
...
You built a **partitioned survival model** — the standard approach for oncology HTA worldwide. The key concepts were:

1. **Survival curve fitting** — using parametric distributions (Weibull) to represent PFS and OS
2. **State partitioning** — deriving health state occupancy from the relationship between OS and PFS
3. **Area-under-the-curve** — total costs and QALYs come from integrating over the survival curves
4. **Half-cycle correction** — applied as in the Markov model
5. **Extrapolation** — extending curves beyond trial data, with different distributions giving different answers

The PSM approach is conceptually simpler than a Markov model for oncology because you work directly with trial survival data. However, it has limitations — it cannot easily model treatment switching, it assumes independence of PFS and OS transitions, and the extrapolation choice is often the single most influential assumption.

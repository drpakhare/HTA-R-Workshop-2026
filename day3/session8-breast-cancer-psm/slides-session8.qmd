...
cat("ICER: ₹", format(round(icer), big.mark=","), "/QALY\n\n")

# Compare with WTP
wtp_1gdp <- 170000  # ~1× GDP per capita for India
if (icer < wtp_1gdp) {
  cat("✓ COST-EFFECTIVE at 1× GDP per capita (₹1,70,000/QALY)\n")
} else {
...
:::notes
This is the bottom line: incremental cost divided by incremental QALY. We compare against Indian WTP thresholds to inform the decision.
:::
...
✓ PSM separates PFS and OS directly from trial data

✓ State occupancy comes from the OS − PFS area relationship

✓ Costs and QALYs accumulate by integrating under the curves

✓ **Extrapolation choice (Weibull vs exponential vs log-logistic) matters enormously**

✓ Always do structural sensitivity analysis on survival distributions
...
:::notes
Key messages. The PSM is the standard oncology approach globally. The extrapolation issue is critical and often underappreciated. PSA will show us how uncertain the ICER really is.
:::

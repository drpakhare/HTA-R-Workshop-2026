...
ggplot(ce_data, aes(x = inc_qaly, y = inc_cost, colour = ce_at_threshold)) +
  ...
  labs(x = "Incremental QALYs", y = "Incremental Cost (₹)",
       title = "CE Plane: Points above the line are not cost-effective",
       caption = "Each point = one iteration") +
  theme_minimal() +
  theme(legend.position = "bottom")
```

:::notes
The CE plane shows the relationship between incremental cost and effect. The WTP line separates cost-effective (below) from not CE (above). Most points cluster below the line.
:::
...
1. **Median/mean ICER** is our best point estimate
2. **95% CrI** shows the plausible range
3. **CEAC at our WTP** shows the probability the intervention is cost-effective
4. **Wide credible intervals** mean decision is uncertain
:::
...
✓ PSA applies the **same methodology** to PSM as to Markov models

✓ Sample parameters → run model → collect results → summarize

✓ **CEAC shows probability of cost-effectiveness** at each WTP threshold

✓ **CE plane visualizes cost vs effect** trade-offs for each iteration

✓ **Wide credible intervals = decision uncertainty**

✓ Survival parameters typically drive the most ICER uncertainty

. . .

Next: Build interactive Shiny apps so stakeholders can explore the model themselves (Session 10)

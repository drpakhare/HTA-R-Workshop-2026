...
cat("ICER (₹/QALY):\n")
cat("  Mean: ₹", format(round(mean(psa_results$icer, na.rm = TRUE), 0), big.mark = ","), "\n", sep = "")
cat("  Median: ₹", format(round(median(psa_results$icer, na.rm = TRUE), 0), big.mark = ","), "\n", sep = "")
cat("  2.5th percentile: ₹", format(round(quantile(psa_results$icer, 0.025, na.rm = TRUE), 0), big.mark = ","), "\n", sep = "")
cat("  97.5th percentile: ₹", format(round(quantile(psa_results$icer, 0.975, na.rm = TRUE), 0), big.mark = ","), "\n", sep = "")

cat("\nIncremental Cost (₹):\n")
cat("  Mean: ₹", format(round(mean(psa_results$inc_cost), 0), big.mark = ","), "\n", sep = "")
cat("  95% CrI: [₹", format(round(quantile(psa_results$inc_cost, 0.025), 0), big.mark = ","),
    ", ₹", format(round(quantile(psa_results$inc_cost, 0.975), 0), big.mark = ","), "]\n", sep = "")

cat("\nIncremental QALY:\n")
cat("  Mean: ", round(mean(psa_results$inc_qaly), 3), "\n", sep = "")
cat("  95% CrI: [", round(quantile(psa_results$inc_qaly, 0.025), 3),
    ", ", round(quantile(psa_results$inc_qaly, 0.975), 3), "]\n", sep = "")
```
...
1. **ICER distribution**: The PSA reveals considerable uncertainty in the ICER. The 95% credible interval is wide, reflecting parameter uncertainty.

2. **Cost-effectiveness at ₹170,000/QALY**: We can calculate the exact probability that trastuzumab is cost-effective at the 1× GDP per capita threshold.

3. **Uncertainty drivers**: Survival uncertainty (reflected in the hazard ratio distributions) has the largest impact on ICER uncertainty. Cost uncertainty, while substantial, has smaller effect.

4. **Decision robustness**: The wide credible interval suggests decision-makers should recognize decision uncertainty when recommending or funding the intervention.
